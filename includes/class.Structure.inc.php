...
class Structure
{
	...
	function url_to_structure()
	{
		...
		if( ENABLE_DEBUG )  {
                	error_log( "url_to_structure Called" );
		}
		/*
		...
		if( ENABLE_DEBUG )  {
                	error_log( "url_to_structure sql = ".$sql );
		}
		...
		{
			if( ENABLE_DEBUG )  {
                    		error_log( "url_to_structure sql query failed" );
                        }
			return FALSE;
		}
	...
	function list_children()
	{
		...
		if( ENABLE_DEBUG )  {
                	error_log( "list_children: sql = ".$sql );
		}
		...
		{
			if( ENABLE_DEBUG )  {
                    		error_log( "list_children: query failed: ".print_r($db->errorInfo(),true) );
			}
			return FALSE;
		}
		...
		while ($child = $result->fetch(PDO::FETCH_OBJ))
		{
			...
			$child->identifier = str_replace('_', '-', $child->s1_identifier);
			
			if( ENABLE_DEBUG )  {
                        	error_log( "list_children: $child->s1_identifier = ".$child->s1_identifier );
                        	error_log( "list_children: $child->identifier = ".$child->identifier );
			}
