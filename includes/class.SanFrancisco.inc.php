...
class Parser
{
	...
	public function __construct($options)
	{
		...
		if($this->directory)
		{
			...
			/*
			 * Iterate through every file in this directory and build up an array of them.
			 */
	 ...
	 */
	public function import_xml($section_data)
	{
		try
		{
		...
		catch(Exception $e)
		{
			...
			if (class_exists('tidy', FALSE))
			{
				...
				$tidy = new tidy();
				$tidy->parseString($section_data, $tidy_config, 'utf8');
				$tidy->cleanRepair();

				$section_data = (string) $tidy;
			}
			 ...
			 */
			$this->section->catch_line = (string) $this->section->catch_line[0];
			$this->section->section_number = (string) $this->section->section_number;
			$this->section->order_by = (string) $this->section->order_by;
			$this->section->history = (string) $this->section->history;
